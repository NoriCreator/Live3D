    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct appdata
            {
                ...
                float2 uv : TEXCOORD0;
                uint instanceID : SV_InstanceID; // URPではこれが必要
            };
            ...
            V2F vert(appdata v)
            {
                ...
                float4 base = basePositionBuffer[id];

                float wave = sin(_TimeY * 3.0 + base.w * 0.1) * 0.5;
                float3 offset = float3(0, wave, 0);

                float4 localPos = float4(v.vertex.xyz + offset, 1.0);
